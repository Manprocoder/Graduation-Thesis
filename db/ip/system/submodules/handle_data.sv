...
(
	...
	assign eot_add_key = text_last & last_cc_i;
	assign pdo = (cs == INIT_PER) | (cs == AD_PER) | (cs == TEXT_PER) | (cs == FINAL_PER);
	assign control_o = {hash_permutation, sqz_hash_do, eot_add_key, final_ct, abs_text_do, abs_ad_do, sep_domain, init_add_key, pdo, start};
	//(2)--TAG
    ...
    always_comb begin
		...
		new_state_o = 64'd0;
		ad_last = 1'b0;
		hash_permutation = 1'b0;
		latch_fifo_wused = 1'b0;
		last_message_block = 1'b0;
        ...
        case(cs)
            ...
            ABS_AD: begin 
				...
				//
				if(hash_flag_i) begin
					hash_permutation = 1'b1;
					//
					if(ad_data_i[68]) begin
						latch_fifo_wused = 1'b1;
						last_message_block = 1'b1;
					end
					else begin
						latch_fifo_wused = 1'b0;
						last_message_block = 1'b0;
				...
				else hash_permutation = 1'b0;
				//
				//change state
				if(run_mode_i == ASCON_128A) begin
                	if(last_cc_i) ns = AD_PER; //abs_ad_done
					else begin
						ad_data_req_o = 1'b1;
						ns = ABS_AD;
					end
            ...
            ABS_TEXT: begin
				...
				//
				if(run_mode_i == ASCON_128A) begin
					if(last_cc_i) begin //abs_text_done
	...
	store_data h0(
		...
		.last_message_block_i(last_message_block),
